...
// Sample to show how the files are loaded and displayed.
-(void)copySampleFilesToDocDirIfNeeded{
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    _documentsDirectory = [[NSString alloc] initWithString:[paths objectAtIndex:0]];
    
    NSString *file1Path = [_documentsDirectory stringByAppendingPathComponent:@"homework.txt"];
    NSString *file2Path = [_documentsDirectory stringByAppendingPathComponent:@"pi.txt"];
    ...
    if (![fileManager fileExistsAtPath:file1Path] || ![fileManager fileExistsAtPath:file2Path]) {
        [fileManager copyItemAtPath:[[NSBundle mainBundle] pathForResource:@"homework" ofType:@"txt"]
                             toPath:file1Path
                              error:&error];
        ...
        [fileManager copyItemAtPath:[[NSBundle mainBundle] pathForResource:@"pi" ofType:@"txt"]
                             toPath:file2Path
                              error:&error];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([[_arrFiles objectAtIndex:indexPath.row] isKindOfClass:[NSString class]]) {
        ...
        cell = [tableView dequeueReusableCellWithIdentifier:@"newFile"];
